...
public class ExtFlightDelaysDAO {
	...
	public List<Adiacenza> getAdiacenze(Integer x, Map<Integer, Airport> idMap) {
		final String sql = "SELECT f.ORIGIN_AIRPORT_ID AS a1,f.DESTINATION_AIRPORT_ID AS a2, COUNT(f.ID) AS peso " + 
				"FROM flights AS f " + 
				...
				"AND f.ORIGIN_AIRPORT_ID IN(SELECT a.ID " + 
				"FROM airports AS a,flights AS f " + 
				"WHERE a.ID = f.ORIGIN_AIRPORT_ID OR a.ID= f.DESTINATION_AIRPORT_ID " + 
				"GROUP BY a.ID " + 
				"HAVING COUNT(DISTINCT(f.AIRLINE_ID))>?) " + 
		...
		try {
			...
			while (rs.next()) {
				Adiacenza a = new Adiacenza(idMap.get(rs.getInt("a1")),idMap.get(rs.getInt("a2")),rs.getInt("peso"));
				lista.add(a);
			}
